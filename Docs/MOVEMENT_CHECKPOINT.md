...
- Press `X` during ordinary play or while aiming to switch between right- and left-shoulder views. The camera mirrors its lateral composition over 0.22 seconds while the crosshair and projectile aim path remain unchanged. The complete authored character visual is evaluated once in its original orientation and its visual root is reflected horizontally only at the symmetric midpoint; its scale never collapses toward zero. During the transition, the torso and bow stance ease to neutral, the bow moves onto the character centerline, and the drawing hand meets the holding hand at the handle. Each intermediate hand target and elbow guide is solved directly; arm rotations are never interpolated between completed endpoint solutions. The midpoint elbow pair remains outward and no higher than either incoming elbow, and a small `±0.10` plateau holds that pose across the orientation change. Hip stance yaw follows the shoulder magnitude directly during the switch so the feet step through neutral instead of exchanging sides in one frame. Ordinary aim and locomotion torso yaw are pre-compensated by the signed shoulder blend before visual reflection, preserving the same world-facing response on both endpoints.
